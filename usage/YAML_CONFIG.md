...
| [`framework/experiments/mstcn2-auto-roi-aug-hand.yaml`](../framework/experiments/mstcn2-auto-roi-aug-hand.yaml) | MS-TCN++（`mstcn2`）h128/s2l5、144 维 ROI 特征、60 轮 / lr 5e-4 / wd 1e-4、`dataset_ref: temporal.actionmixed-auto-roi-v1`；`augmentation.target_mask` **启用**：`strategy: frame_dropout`、**只遮 `hand` 一类**、`probability: 0.5` | 训练期**随机目标遮罩（单类版）**对照：针对「批次特定线索记忆」病灶（flush 依赖的 `hand.left-top` 在 train 出现率 0.448、test 为 **0.000**）。遮罩**只作用于 train**，val/test 输入干净，同 seed 可复现；与全类版共用 p=0.5 做对照（依据 `docs/FEATURE_STRATEGY_COMPARE.md` 第十三/十九/二十轮）。 |
| [`framework/experiments/mstcn2-auto-roi-aug-all.yaml`](../framework/experiments/mstcn2-auto-roi-aug-all.yaml) | 上条的**全检测类**版：`targets` 覆盖 8 个检测类，每采样帧、每个目标独立按 `probability: 0.5` 清零其整类特征块（ROI-144 下 18 维）；其余字段与单类版逐项一致 | 单类版的正交对照：逐帧随机抹掉一部分检测证据，迫使模型依赖「多帧 + 多类共同出现」的时序模式而非任一单帧单类线索。**不变量**：`probability` 过高会退化成「输入大面积置零」，故与单类版同用 0.5。 |
