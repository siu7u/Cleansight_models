...
`benchmark/testsets.yaml` 目前没有“等价类”字段，只登记数据集、类别、split 和用途。
...
- “图片/有框/空标/缺标”按本机 `datasets/yolo/**` 当前文件统计；
...
PYTHONDONTWRITEBYTECODE=1 python tools/validate_testsets.py \
  --catalog benchmark/testsets.yaml --json
```

```bash
git ls-files datasets/yolo/group1_large datasets/yolo/group2_small
git check-ignore -v datasets/yolo/group1_large/data.yaml
```
