# 模型卡：auto-mstcn-v1（自动标注数据通道）

## 版本钉定

- 模型版本：v1
- 模型类型：MS-TCN（全序列，非因果）
- 权重：`runs/mstcn-20260817-170254/checkpoints/best.pt`
  （sha256: `b458f23b14bc7484af05...`，完整值见 `pin.yaml`）
...
- 因果性：否（全序列离线推理）
- 输入形状：`[1, T, 40]`，`input_dim=40`，`num_classes=6`
- 参数量：见 run `config.resolved.json` 与 checkpoint meta
- 训练配置：`framework/experiments/mstcn-actionmixed-auto.yaml`（30 epoch）

## 评估结果（formal，2026-08-17）

- 训练期验证集 best val_acc：16.18（epoch 2）
- test split（98 帧、仅 long_brush_withdraw）：acc 0.0 / edit 0.0
  —— 见 `docs/AUTO_ANNOTATION.md`「正式训练结果」：0.0 是 test 集单类弱势类别的
  真实结果，不代表链路问题
- 评测报告：`runs/mstcn-20260817-170254/evals/*.evaluation.json`、
  `runs/mstcn-20260817-170254/checkpoints/EVALUATION_REPORT.md`

## 已知限制

- 检测特征仅覆盖 6 类（short_brush / brush_tip_out 恒零），40 维 v1 布局兼容
- test 集为 98 帧单类样本，正式 benchmark 意义有限；待项目 16 新标注视频到位后重划 split
