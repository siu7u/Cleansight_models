"""YOLO 自动标注推理管线：视频 → legacy 时序标注 JSON。
...
标注新视频自动生成检测标注（时序模型特征输入）。格式约定见
``auto_annotate.legacy_format``。
...
from ._constants import DEFAULT_TOP_K
...
def _build_sequence(by_frame: dict[int, dict], frames_count: int, fps: float) -> list[dict]:
    ...
    for frame_index in range(frames_count):
        ...
        if detection is not None:
            ...
            x, y, width, height = legacy_format.ls_box_from_xywhn(last_box)
            entry.update(
                {
                    "enabled": True,
                    "x": x,
...
def _normalize_conf(conf: float | dict) -> tuple[float, dict | None]:
    """把 conf 规范化为 ``(推理阈值, 类别级阈值或 None)``。

    标量全局生效；dict 时按最低阈值推理、逐类过滤（低于类别阈值的检测丢弃）。
    ``dataset.run_dataset_annotate`` 复用同一口径。
    """

    if isinstance(conf, dict):
        if not conf:
            raise ValueError("conf 字典不能为空")
        return min(conf.values()), {name: float(t) for name, t in conf.items()}
    return float(conf), None


def _load_models(
    checkpoint_specs: list[dict], *, imgsz: int, runs_dir: Path | None
) -> list:
    """加载并校验 checkpoint 列表，返回 ``[(model, class_map), ...]``。
...
def run_auto_annotate(
    ...
    *,
    ...
    for task_id, video in enumerate(videos):
        ...
        task = legacy_format.build_task(video.name, tracks, total, fps, task_id)
