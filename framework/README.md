# cleansight_eval —— 多条流水线的训练与推理框架

本目录是对 `docs/TRAIN_EVAL_REQUIREMENTS.md` 的一次落地。**架构是多条完整流水线 + 一条薄
公共层**。各 pipeline 只负责训练和预测；benchmark 在外层调用预测能力并独占正式评测：
...
| ROI 图像分类 | `roi_classification` | ROI 图像块 + 多标签 | 多标签 P/R/F1 | N/A |
...
| 层 | 目录 | 职责 |
|---|---|---|
| **公共层** | `cleansight_eval/core/` | run 组织、配置、环境、checkpoint 守卫、Pipeline 注册、模型执行事实 `PredictionOutput`、数据契约 `catalog`、指标原语 `metrics` |
| **评测层** | `../benchmark/` | 评测 CLI、固定 testset 口径消费、指标三态翻译、`EvaluationResult v2`、prediction artifact、报告、矩阵与交付 manifest |
...
> 训练/评测共享的数据契约（`framework/testsets.yaml` → `core/catalog.py`）与指标原语
> （`core/metrics.py`）都归 framework，benchmark 消费它们。
>
> 时序共享的 `data`/`util` **只在两条时序流水线间复用**，绝不跨到 detection——
> 检测输入是图像、由 ultralytics 从 `data.yaml` 自持读入，与时序的特征序列是两套不相交
...
python -m benchmark.cli.eval --config framework/experiments/yolo-clean-large.yaml \
...
- `formal`：testset 必须在 `framework/testsets.yaml` 登记并通过校验；metadata 必须与 checkpoint
...
- `formal testset validation failed`：先运行
  `python tools/validate_testsets.py --catalog framework/testsets.yaml --json`。数据充足时应修复切分；小数据
