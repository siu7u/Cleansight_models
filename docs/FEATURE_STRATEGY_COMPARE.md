...
> CPU 环境内可复现。2026-09-04 在 GPU（RTX 4060 Laptop，数据根 `-lhh`）复跑 roi-grid-144
> （`runs/formal_roi_20260905/`）：三 seed 零坍缩不变但逐 seed 漂移明显——seed 42 F1@0.1
> 33.3→44.9，seed 7 28.6→19.1，seed 2026 31.8→19.5；中位 edit/F1@0.1/F1@0.25 =
> 23.59/19.51/14.63。**设备差异大于单轮噪声：跨设备比较只能定性**；GPU 全策略矩阵待跑后
> 更新横向结论。
