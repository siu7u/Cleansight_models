...
- 帧级 acc 全面下降（25~28%）是**健康信号**：不再"永远猜 idle"（acc 在 65%+ idle 数据上
  是误导指标，段级 edit/F1 才是主线）。
- **段级 edit/F1@0.1：A 整个画面最好**；F1@0.25/F1@0.5 上 C/B 与 A 互有胜负但样本量小。
- B 修复后不再是吊车尾——第一轮"B 最弱"的结论是坍缩污染，作废。

运行目录（未入库）：`tmp/try_fix/gru-20260903-{135238,135327,135413}/`。
