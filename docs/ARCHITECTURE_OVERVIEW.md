...
## 2. Framework：负责运行模型与数据契约

[`framework/cleansight_eval`](../framework/cleansight_eval/) 提供统一模型执行能力，并分成多层：

```text
cli/        训练与优化实验命令（train / sweep）
core/       配置、checkpoint、run、Pipeline 注册、PredictionOutput、
            数据契约 catalog、指标原语 metrics
temporal/   时序数据、特征、模型、训练和预测
detection/  YOLO 训练、checkpoint 加载、逐图预测、优化实验编排（sweep）、数据集裁剪（data_tools）
classification/  ROI 图像分类（特征融合）：模型、ROI 数据集、训练/预测
```
...
| `roi_classification` | ROI 图像块 + 多标签，小目标/稀有类的特征融合替代方案。 |

Pipeline 只负责训练和预测，输出统一的 `PredictionOutput`，不在内部定义正式指标。

**依赖方向（强制执行）**：`benchmark → framework` 单向。framework 生产代码不 import
benchmark；训练与评测共享的数据契约（`framework/testsets.yaml` → `core/catalog.py`）与
指标原语（`core/metrics.py`）归 framework，benchmark 消费它们。

## 3. Benchmark：负责数据身份与评判

[`benchmark`](../benchmark/) 负责：

- 提供单 checkpoint 评测、矩阵、逐类分析（`analyze`）和历史结果升级入口；
- 消费 [`framework/testsets.yaml`](../framework/testsets.yaml) 登记的数据集版本、split、manifest 和重叠策略；
- 计算 Accuracy、Edit、F1、Precision、Recall、Temporal IoU、YOLO 指标和多标签 P/R/F1；
...
| YOLO 多方法优化实验（预设/grid） | `framework/cleansight_eval/detection/sweep.py` |
| 小目标逐类分析/淘汰决策 | `benchmark/cli/analyze.py` + `benchmark/core/analysis.py` |
| ROI 特征融合 | `framework/cleansight_eval/classification/` + `benchmark/evaluators/classification.py` |
...
| 修改训练/推理方式 | 对应 temporal / detection / classification Pipeline |
| 修改指标定义 | `framework/cleansight_eval/core/metrics.py` 或 evaluator |
| 修改固定数据集/split | `framework/testsets.yaml` 和 manifest |
| 修改本地数据挂载 | `datasets/` 与 `framework/testsets.yaml` |
